...
AdvecDiff2D::AdvecDiff2D(const double in_ad_vel_x, const double in_ad_vel_y, const double in_alpha,const unsigned int in_m, const double in_dt):
    ad_vel_x(in_ad_vel_x), ad_vel_y(in_ad_vel_y), alpha(in_alpha), m(in_m), dt(in_dt), M(in_m * in_m)
{
    ...
    {
        for(auto j = 0; j < M.get_columns(); j++)
        {
...
Vector<double> AdvecDiff2D::solve(double time_end) const
{
    const clock_t begin_time = std::clock();
